<div class="mx-2 mb-8">
    <article
        ...
        @endif
        <div
            class="absolute inset-0 bg-gradient-to-t dark:from-gray-900 dark:via-gray-900/40 from-teal-900 via-teal-900/40"></div>
        <h3 class="z-10
         {{$titleClasses ?? 'mt-2 text-2xl'}}
            ...
            <div class="z-10 mt-6 absolute bottom-4 right-4">
                <div class="flex flex-row">
                    @if (isset($deleteButton))
                        <x-danger-button wire:confirm="Are you sure you want to delete this?"
                                         wire:click="delete({{ $deleteButton['id'] }})"
                                         class="bg-red-500 text-white rounded-lg hover:bg-red-600 transition duration-300">
                            Delete
                        </x-danger-button>
                    @endif
